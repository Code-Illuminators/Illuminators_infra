...
echo "CONSUL_RETRY_JOIN=$CONSUL_RETRY_JOIN"

exec consul agent -server -bootstrap-expect=3 -ui -bind="$CONSUL_BIND_IP" -client=0.0.0.0 -config-dir=/consul/config -data-dir=/consul/data -datacenter=dc1 -client=0.0.0.0 -retry-join="$CONSUL_RETRY_JOIN"